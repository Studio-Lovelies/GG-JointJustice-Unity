...
namespace Tests.PlayModeTests.Scripts.PauseMenu
{
    public class ViaMouse : PauseMenuTests
    {
        ...
        public IEnumerator PauseMenuCanBeNavigated()
        {
            ...
            {
                yield return inputTestTools.SetMouseWorldSpacePosition(GetButtonCenter(button));
        ...
        {
            yield return TestResumeButton(() => inputTestTools.ClickAtWorldSpacePosition(GetButtonCenter(GetButton(RESUME_BUTTON_NAME))));
        }
        ...
        {
            yield return TestSettingsButton(() => inputTestTools.ClickAtWorldSpacePosition(GetButtonCenter(GetButton(SETTINGS_BUTTON_NAME))));
        }
        ...
        {
            yield return TestMainMenuButton(() => inputTestTools.ClickAtWorldSpacePosition(GetButtonCenter(GetButton(MAIN_MENU_BUTTON_NAME))));
        }
        ...
        {
            yield return ToggleMenu();
            var button = GetButton(RESUME_BUTTON_NAME);
            AssertButtonSelected(button);
            yield return inputTestTools.ClickAtWorldSpacePosition(new Vector2(10, 10));
            AssertButtonSelected(button);
        }
